"""Business-domain service seed for Attendance Record."""

from __future__ import annotations


ARCHETYPE_PROFILE = {'workflow_profile': {'mode': 'transaction_flow', 'supports_submission': True}, 'reporting_profile': {'supports_snapshots': True, 'supports_outputs': True}, 'integration_profile': {'external_sync_enabled': True, 'tracks_external_refs': True}, 'lifecycle_states': ['draft', 'approved', 'archived'], 'is_transactional': True}

CONTRACT = {'title_field': 'title', 'status_field': 'workflow_state', 'reference_field': 'reference_no', 'required_fields': ['title', 'workflow_state', 'transaction_date'], 'field_purposes': {'workflow_state': 'lifecycle_state', 'transaction_date': 'transaction_date', 'party': 'primary_party', 'currency': 'currency_code', 'total_amount': 'total_amount', 'employee': 'party_reference', 'approval_status': 'status_flag', 'related_payroll_run': 'relation_collection'}, 'search_fields': ['title', 'reference_no', 'description', 'attendance_number', 'employee', 'period'], 'list_columns': ['title', 'reference_no', 'transaction_date', 'party', 'total_amount', 'workflow_state'], 'initial_state': 'draft', 'lifecycle_states': ['draft', 'approved', 'archived'], 'terminal_states': ['archived'], 'action_targets': {'create': None, 'review': None, 'approve': 'approved', 'archive': 'archived'}}

WORKFLOW_HINTS = {'business_objective': 'calculate, approve, pay, document, and post payroll accurately for a pay period', 'actors': ['payroll officer', 'HR reviewer', 'approver', 'finance officer', 'employee'], 'start_condition': 'a payroll period is due for processing', 'ordered_steps': ['Collect attendance and salary basis.'], 'primary_actions': ['record', 'review', 'update'], 'primary_transitions': [], 'downstream_effects': ['payroll history becomes available for employee records, finance controls, and reporting outputs'], 'action_actors': {'create': ['payroll officer'], 'review': ['HR reviewer'], 'approve': ['approver'], 'archive': ['payroll officer']}}

SIDE_EFFECT_HINTS = {'downstream_effects': ['payroll history becomes available for employee records, finance controls, and reporting outputs'], 'related_docs': ['employee_record', 'payroll_run'], 'action_targets': {'create': None, 'review': None, 'approve': 'approved', 'archive': 'archived'}, 'action_side_effects_file': 'side_effects.json'}

class DomainService:
    doc_id = "attendance_record"
